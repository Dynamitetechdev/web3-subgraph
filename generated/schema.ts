...
export class nftId extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type nftId must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("nftId", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): nftId | null {
    return changetype<nftId | null>(store.get("nftId", id.toHexString()));
  }
